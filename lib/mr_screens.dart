          ...
          child: StreamBuilder<QuerySnapshot>(
            ...
            builder: (context, snapshot) {
              ...
              final docs = snapshot.data!.docs.where((d) {
                ...
                return active && (_search.isEmpty || name.contains(_search)) && (_filterDiv == 'All' || div == _filterDiv);
              }).toList();
                  ...
                  return Card(
                    ...
                    child: ListTile(
                      ...
                      trailing: Column(mainAxisAlignment: MainAxisAlignment.center, crossAxisAlignment: CrossAxisAlignment.end, children: [
                        Text('$stock units', style: TextStyle(fontWeight: FontWeight.bold, color: stockColor)),
                        Container(
                  ...
                  const Center(child: Padding(padding: EdgeInsets.all(20), child: CircularProgressIndicator()))
                    ...
                    Column(
                      children: _products.map((p) {
                        ...
                        Color stockColor;
                        String stockLabel;
                        if (stockistQty == 0) { stockColor = Colors.red; stockLabel = 'No stock'; }
                        else if (stockistQty < 10) { stockColor = Colors.orange; stockLabel = '$stockistQty units'; }
                        else { stockColor = Colors.green; stockLabel = '$stockistQty units'; }
                        return Card(
                            ...
                            child: Row(children: [
                              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                                ...
                                Row(children: [
                                  ...
                                  Container(
                                    padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2),
                                    decoration: BoxDecoration(color: stockColor.withOpacity(0.1), borderRadius: BorderRadius.circular(8), border: Border.all(color: stockColor.withOpacity(0.4))),
                                    child: Text('Stockist: $stockLabel', style: TextStyle(fontSize: 10, color: stockColor, fontWeight: FontWeight.w600)),
                                  ),
                                ]),
                              ])),
                              Row(children: [
                                IconButton(onPressed: qty > 0 ? () => setState(() => _quantities[p.id] = qty - 1) : null, icon: const Icon(Icons.remove_circle_outline), color: const Color(0xFF1565C0), iconSize: 22),
                                SizedBox(width: 32, child: Text('$qty', textAlign: TextAlign.center, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold))),
                                IconButton(onPressed: () => setState(() => _quantities[p.id] = qty + 1), icon: const Icon(Icons.add_circle_outline), color: const Color(0xFF1565C0), iconSize: 22),
                              ]),
                            ]),
                          ),
                        );
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          TextButton(
            onPressed: () async {
              final unread = await db.collection('notifications').where('mrId', isEqualTo: uid).where('isRead', isEqualTo: false).get();
              for (final doc in unread.docs) { await doc.reference.update({'isRead': true}); }
            },
      ...
      body: StreamBuilder<QuerySnapshot>(
        stream: db.collection('notifications').where('mrId', isEqualTo: uid).orderBy('createdAt', descending: true).snapshots(),
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) return const Center(child: CircularProgressIndicator());
          if (!snapshot.hasData || snapshot.data!.docs.isEmpty) {
            return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
            ...
            itemBuilder: (context, i) {
              ...
              return Card(
                ...
                child: ListTile(
                  ...
                  title: Text(data['title'] ?? '', style: TextStyle(fontWeight: isRead ? FontWeight.normal : FontWeight.bold, fontSize: 14)),
                  ...
                  onTap: () async { if (!isRead) await docs[i].reference.update({'isRead': true}); },
                ),
              );
            },
          );
